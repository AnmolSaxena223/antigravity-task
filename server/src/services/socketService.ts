...
import { GameSession, IGameSession, IGamePlayer } from '../models/GameSession';
import { Transaction } from '../models/Transaction';
import { Friend } from '../models/Friend';
import { executeTokenMove, hasValidMoves, isValidMove, mapStepToCommonTrack, isCellSafe } from '../utils/ludoEngine';
import crypto from 'crypto';
import { Types } from 'mongoose';

interface ActiveRoomTimer {
  rollTimeout?: NodeJS.Timeout;
...
export class SocketService {
  ...
  public static activeUsers: Map<string, string> = new Map(); // userId -> socketId
  private matchmakingQueue: { userId: string; entryFee: number; socketId: string }[] = [];
  private activeTimers: Map<string, ActiveRoomTimer> = new Map(); // roomId -> timers
  ...
  private setupConnection() {
    this.io.on('connection', (socket: Socket) => {
      ...
      // Matchmaking
      socket.on('matchmake:start', (data: { entryFee: number }) => this.handleMatchmaking(socket, userId, data.entryFee));
      socket.on('matchmake:cancel', () => this.handleCancelMatchmaking(userId));

      // Private Rooms
      socket.on('room:create', (data: { entryFee: number }) => this.handleCreatePrivateRoom(socket, userId, data.entryFee));
      socket.on('room:join', (data: { roomId: string }) => this.handleJoinPrivateRoom(socket, userId, data.roomId));
      socket.on('room:join_lobby', (data: { roomId: string }) => {
      ...
      socket.on('game:move', (data: { tokenId: number }) => this.handleTokenMove(socket, userId, data.tokenId));
      socket.on('game:chat', (data: { message: string }) => this.handleChat(socket, userId, data.message));
   ...
   */
  private async deductEntryFee(userId: string, amount: number, session: any = null): Promise<boolean> {
   ...
   */
  private async handleMatchmaking(socket: Socket, userId: string, entryFee: number) {
    try {
      // Validate balance
      const user = await User.findById(userId);
      ...
      if (potentialMatches.length >= 1) {
        ...
        if (!deductedUser || !deductedMatch) {
          ...
          if (deductedUser) {
            await User.findByIdAndUpdate(userId, { $inc: { 'balance.deposit': entryFee } });
          }
          if (deductedMatch) {
            await User.findByIdAndUpdate(match.userId, { $inc: { 'balance.deposit': entryFee } });
          }
        ...
        const roomId = 'ROOM' + crypto.randomBytes(3).toString('hex').toUpperCase();
        
        const player1 = await User.findById(userId);
        const player2 = await User.findById(match.userId);
  ...
  private handleCancelMatchmaking(userId: string) {
   ...
   */
  private async checkAndStartBotMatch(userId: string, entryFee: number) {
    try {
      // Check if user is still in matchmaking queue
      const queueIndex = this.matchmakingQueue.findIndex(q => q.userId === userId);
      if (queueIndex === -1) return; // User already matched or cancelled
   ...
   */
  private async handleCreatePrivateRoom(socket: Socket, userId: string, entryFee: number) {
    try {
      const user = await User.findById(userId);
      if (!user) return socket.emit('error', 'User not found');
      ...
      if (!deducted) return socket.emit('room:error', 'Balance deduction failed.');
   ...
   */
  private async handleJoinPrivateRoom(socket: Socket, userId: string, roomId: string) {
    try {
      const game = await GameSession.findOne({ roomId, status: 'waiting' });
      if (!game) {
      ...
      if (!deducted) return socket.emit('room:error', 'Fee deduction failed.');
   ...
   */
  private async handleDiceRoll(socket: Socket, userId: string) {
    try {
      const game = await GameSession.findOne({ status: 'active', 'players.userId': userId });
      if (!game) return socket.emit('error', 'Game session not found.');
      ...
      const rolledValue = crypto.randomInt(1, 7);
      ...
      const movesAvailable = hasValidMoves(activePlayer, rolledValue);
   ...
   */
  private async handleTokenMove(socket: Socket, userId: string, tokenId: number) {
    try {
      const game = await GameSession.findOne({ status: 'active', 'players.userId': userId });
      ...
      if (!isValidMove(activePlayer, tokenId, game.diceValue)) {
      ...
      // Execute movement
      const moveResult = executeTokenMove(game.players, game.turnIndex, tokenId, game.diceValue);
      ...
      if (moveResult.gameCompleted) {
        ...
        for (const loser of loserPlayers) {
          const u = await User.findById(loser.userId);
          if (u) {
        ...
        game.turn = new Types.ObjectId(moveResult.nextPlayerId);
   ...
   */
  private handleChat(socket: Socket, userId: string, message: string) {
   ...
   */
  private async startTurnTimers(roomId: string, mode: 'roll' | 'move' = 'roll') {
    this.clearRoomTimers(roomId);

    try {
      const game = await GameSession.findOne({ roomId, status: 'active' });
      if (game) {
        const activePlayer = game.players[game.turnIndex];
    ...
    const roomTimers: ActiveRoomTimer = {};
   ...
   */
  private async checkAndExecuteBotTurn(roomId: string) {
    try {
      const game = await GameSession.findOne({ roomId, status: 'active' });
      if (!game) return;

      const activePlayer = game.players[game.turnIndex];
      if (activePlayer.name !== 'AI Bot') return;
   ...
   */
  private async executeBotAction(roomId: string) {
    try {
      const game = await GameSession.findOne({ roomId, status: 'active' });
      if (!game) return;
   ...
   */
  private async executeBotMove(roomId: string) {
    try {
      const game = await GameSession.findOne({ roomId, status: 'active' });
      if (!game) return;
      ...
      const validMoves: { tokenId: number; score: number }[] = [];

      activePlayer.tokens.forEach((token) => {
        if (!isValidMove(activePlayer, token.id, diceValue)) {
          return;
        }
        ...
        const toGlobalCell = mapStepToCommonTrack(activePlayer.color, toPos);
        if (toGlobalCell !== -1 && !isCellSafe(toGlobalCell)) {
          let hasCapture = false;
          for (let pIdx = 0; pIdx < game.players.length; pIdx++) {
            if (pIdx === game.turnIndex) continue;
            const otherPlayer = game.players[pIdx];
            for (const otherToken of otherPlayer.tokens) {
              const otherGlobalCell = mapStepToCommonTrack(otherPlayer.color, otherToken.position);
      ...
      const moveResult = executeTokenMove(game.players, game.turnIndex, selectedTokenId, diceValue);
      ...
      if (moveResult.gameCompleted) {
        ...
        if (humanPlayer) {
          const u = await User.findById(humanPlayer.userId);
          if (u) {
            u.gameStats.played += 1;
            u.gameStats.lost += 1;
            await u.save();
        ...
        const botUser = await User.findById(activePlayer.userId);
        if (botUser) {
   ...
   */
  private async handleRollTimeout(roomId: string) {
    try {
      const game = await GameSession.findOne({ roomId, status: 'active' });
      if (!game) return;
      ...
      if (activePlayer.skipCount >= 3) {
        ...
        const rollVal = crypto.randomInt(1, 7);
        ...
        const movesAvailable = hasValidMoves(activePlayer, rollVal);
        await game.save();
   ...
   */
  private async handleMoveTimeout(roomId: string) {
    try {
      const game = await GameSession.findOne({ roomId, status: 'active' });
      if (!game) return;
      ...
      if (activePlayer.skipCount >= 3) {
        ...
        for (const token of activePlayer.tokens) {
          if (isValidMove(activePlayer, token.id, game.diceValue || 1)) {
        ...
        if (selectedTokenId !== -1) {
          const fromPos = activePlayer.tokens.find(t => t.id === selectedTokenId)!.position;
          const moveResult = executeTokenMove(game.players, game.turnIndex, selectedTokenId, game.diceValue || 1);
          ...
          if (moveResult.gameCompleted) {
            ...
            const prize = game.prizePool;
            const winner = await User.findById(activePlayer.userId);
            if (winner) {
              winner.balance.win += prize;
              winner.gameStats.played += 1;
              winner.gameStats.won += 1;
   ...
   */
  private async handlePlayerResign(game: IGameSession, resignUserId: string) {
    game.status = 'completed';
    ...
    // Loser stats update
    const loser = await User.findById(resignUserId);
    if (loser) {
   ...
   */
  private async transitionTurn(roomId: string) {
    try {
      const game = await GameSession.findOne({ roomId, status: 'active' });
      if (!game) return;
      ...
      do {
        ...
        game.players[nextIdx].tokens.every(t => t.position === 56) &&
   ...
   */
  private clearRoomTimers(roomId: string, type: 'all' | 'roll' | 'move' = 'all') {
   ...
   */
  private getGameSyncPayload(game: IGameSession) {
    return {
      roomId: game.roomId,
      players: game.players,
      status: game.status,
