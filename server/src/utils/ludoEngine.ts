import { IGamePlayer, IPlayerToken } from '../models/GameSession';
 ...
 */
export const mapStepToCommonTrack = (color: 'red' | 'green' | 'yellow' | 'blue', step: number): number => {
 ...
 */
export const isCellSafe = (cellIndex: number): boolean => {
  return SAFE_CELLS.includes(cellIndex);
};

/**
 * Checks if a player has any valid moves available for the rolled dice value.
 */
export const hasValidMoves = (player: IGamePlayer, diceValue: number): boolean => {
 ...
 */
export const isValidMove = (player: IGamePlayer, tokenId: number, diceValue: number): boolean => {
...
export interface MoveResult {
  success: boolean;
  capturedTokens: { playerId: string; color: string; tokenId: number }[];
...
export const executeTokenMove = (
  ...
  // Update token safety
  const globalCell = mapStepToCommonTrack(activePlayer.color, newPosition);
  token.isSafe = globalCell !== -1 ? isCellSafe(globalCell) : true;
  ...
  const capturedTokens: { playerId: string; color: string; tokenId: number }[] = [];

  // Check captures if landing on common track
  if (globalCell !== -1 && !isCellSafe(globalCell)) {
    for (let pIdx = 0; pIdx < players.length; pIdx++) {
      if (pIdx === activePlayerIndex) continue; // Skip own tokens
      ...
      for (const otherToken of otherPlayer.tokens) {
        ...
        if (otherGlobalCell === globalCell) {
          ...
          capturedTokens.push({
            playerId: (otherPlayer.userId as any).toString(),
  ...
  if (!getsExtraRoll) {
    ...
    do {
      ...
      players[nextPlayerIndex].tokens.every(t => t.position === 56) &&
  ...
  return {
    ...
    capturedTokens,
    tokenReachedHome,
    nextPlayerId: (players[nextPlayerIndex].userId as any).toString(),
    gameCompleted
  };
};
